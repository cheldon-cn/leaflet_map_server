...
class RenderContext {
public:
    RenderContext(int width, int height, const Envelope& bounds);
...
class MapService {
public:
    MapService(std::shared_ptr<renderer::Renderer> renderer,
               std::shared_ptr<cache::MemoryCache> cache,
               const Config& config);
    
    ServiceResult GetTile(int z, int x, int y, 
...
**说明**: HTTP Server 已实现基本功能，需要与 MapService 集成以实现完整的业务逻辑
...
private:
    ...
    Config config_;
    std::unique_ptr<database::IDatabase> database_;
    std::unique_ptr<encoder::IEncoder> encoder_;
    std::shared_ptr<cache::MemoryCache> cache_;
    std::shared_ptr<renderer::Renderer> renderer_;
...
**依赖**: Task 1.2, Task 1.3, Task 2.4, Task 3.4, Task 4.1, Task 5.2, Task 7.1
...
### Task 8.4: CMakeLists.txt 项目构建配置
**优先级**: P0  
**预计工时**: 4小时  
**状态**: [ ] 待执行
...
**新增任务**:
1. **Task 8.4**: CMakeLists.txt 项目构建配置 - 补充遗漏的构建配置任务

**调整说明**:
1. **Task 5.1 (渲染上下文)**: 从原 Task 5.2 拆分出来，独立为一个任务
2. **Task 8 (测试部署)**: 拆分为 3 个独立任务（单元测试、部署文档、CMakeLists）

**可选任务**:
1. **Task 2.3 (PostgreSQL 适配器)**: 适用于生产环境需要连接 PostgreSQL/PostGIS 的场景
2. **Task 3.3 (WebP 编码器)**: 需要 libwebp 库支持，适用于需要更高压缩率的场景
3. **Task 4.2 (磁盘缓存)**: 适用于需要存储更多历史瓦片数据的场景
