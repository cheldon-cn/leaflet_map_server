...
| ogc_layer (图层) | ~20 | DataSource/DriverManager/RasterLayer等未使用 |
...
| API函数 | 模块 | 理由 |
|---------|------|------|
| `ogc_logger_get_instance` | ogc_base | 日志是基础功能，JNI层需要日志输出 |
| `ogc_logger_set_level` | ogc_base | 控制日志级别 |
| `ogc_logger_info` | ogc_base | JNI层日志输出 |
| `ogc_logger_error` | ogc_base | JNI层错误日志 |
| `ogc_coord_transformer_create` | ogc_proj | 坐标转换是海图核心功能 |
| `ogc_coord_transformer_transform` | ogc_proj | 坐标转换执行 |
| `ogc_coord_transformer_destroy` | ogc_proj | 资源释放 |
| `ogc_ais_target_create` | ogc_navi | AIS目标是航海核心功能 |
| `ogc_ais_target_get_latitude` | ogc_navi | AIS位置获取 |
| `ogc_ais_target_get_longitude` | ogc_navi | AIS位置获取 |
| `ogc_ais_manager_create` | ogc_navi | AIS管理是航海核心功能 |
| `ogc_ais_manager_get_target` | ogc_navi | AIS目标查询 |
| `ogc_route_create` | ogc_navi | 航线是航海核心功能（注意：当前为桩实现） |
| `ogc_route_add_waypoint` | ogc_navi | 航线编辑 |
| `ogc_waypoint_create` | ogc_navi | 航路点是航海核心功能 |
| `ogc_iparser_create` | ogc_parser | 海图解析是核心功能（注意：当前为桩实现） |
| `ogc_iparser_open` | ogc_parser | 打开海图文件 |
| `ogc_iparser_parse` | ogc_parser | 解析海图 |
| `ogc_tile_cache_create` | ogc_cache | 瓦片缓存对性能至关重要 |
| `ogc_tile_cache_get_tile` | ogc_cache | 瓦片获取 |
| `ogc_alert_engine_create` | ogc_alert | 警报引擎是安全功能 |
| `ogc_alert_engine_check_all` | ogc_alert | 安全检查 |

#### P1 - 中优先级（增强功能，建议补充）

| API函数 | 模块 | 理由 |
|---------|------|------|
| `ogc_multipoint_create` | ogc_geom | 多点几何类型完整性 |
| `ogc_multilinestring_create` | ogc_geom | 多线几何类型完整性 |
| `ogc_multipolygon_create` | ogc_geom | 多面几何类型完整性 |
| `ogc_geometry_collection_create` | ogc_geom | 几何集合完整性 |
| `ogc_geometry_factory_create` | ogc_geom | 几何工厂模式 |
| `ogc_geometry_factory_create_from_wkt` | ogc_geom | WKT解析 |
| `ogc_geometry_factory_create_from_geojson` | ogc_geom | GeoJSON解析 |
| `ogc_datasource_open` | ogc_layer | 数据源管理 |
| `ogc_datasource_close` | ogc_layer | 数据源释放 |
| `ogc_datasource_get_layer_count` | ogc_layer | 数据源图层查询 |
| `ogc_datasource_get_layer` | ogc_layer | 数据源图层获取 |
| `ogc_color_from_rgb` | ogc_draw | 颜色创建 |
| `ogc_color_from_rgba` | ogc_draw | 颜色创建（含透明度） |
| `ogc_font_create` | ogc_draw | 字体创建 |
| `ogc_pen_create` | ogc_draw | 画笔创建 |
| `ogc_brush_create` | ogc_draw | 画刷创建 |
| `ogc_draw_style_create` | ogc_draw | 绘制样式创建 |
| `ogc_symbolizer_create` | ogc_symbology | 符号化器 |
| `ogc_symbolizer_rule_create` | ogc_symbology | 符号化规则 |
| `ogc_filter_create` | ogc_symbology | 过滤器 |
| `ogc_offline_storage_create` | ogc_cache | 离线存储 |
| `ogc_offline_storage_create_region` | ogc_cache | 离线区域创建 |
| `ogc_track_create` | ogc_navi | 航迹记录 |
| `ogc_track_recorder_create` | ogc_navi | 航迹记录器 |
| `ogc_cpa_calculate` | ogc_alert | CPA计算 |
| `ogc_ukc_calculate` | ogc_alert | UKC计算 |
| `ogc_nmea_parser_create` | ogc_navi | NMEA解析 |
| `ogc_position_provider_create_serial` | ogc_navi | 串口定位 |
| `ogc_performance_monitor_get_instance` | ogc_base | 性能监控 |
| `ogc_performance_stats_get` | ogc_base | 性能统计 |

#### P2 - 低优先级（辅助功能，可后续补充）

| API函数 | 模块 | 理由 |
|---------|------|------|
| `ogc_circuit_breaker_create` | ogc_recovery | 容错功能（当前C++层未实现） |
| `ogc_graceful_degradation_create` | ogc_recovery | 降级功能（当前C++层未实现） |
| `ogc_error_recovery_manager_register_strategy` | ogc_recovery | 错误恢复（当前C++层未实现） |
| `ogc_health_check_create` | ogc_health | 健康检查 |
| `ogc_health_check_execute` | ogc_health | 健康检查执行 |
| `ogc_library_loader_create` | ogc_loader | 动态库加载 |
| `ogc_secure_library_loader_create` | ogc_loader | 安全库加载 |
| `ogc_chart_exception_create` | ogc_exception | 异常创建 |
| `ogc_chart_exception_get_message` | ogc_exception | 异常消息获取 |
| `ogc_plugin_manager_create` | plugin | 插件管理 |
| `ogc_collision_assessor_create` | ogc_navi | 碰撞评估 |
| `ogc_off_course_detector_create` | ogc_navi | 偏航检测 |
| `ogc_data_encryption_create` | ogc_cache | 数据加密 |
| `ogc_cache_manager_create` | ogc_cache | 缓存管理器 |
| `ogc_render_optimizer_create` | ogc_graph | 渲染优化 |
| `ogc_transform_matrix_create` | ogc_proj | 变换矩阵 |
...
| 4 | 航线/航路点 | 15 | 部分桩实现，需先完善C++层 |
| 5 | 海图解析器 | 8 | 部分桩实现，需先完善C++层 |
...
1. **当前JNI覆盖率较低**（15.8%），主要集中在ChartViewer、Viewport、Geometry、Feature、Layer等基础模块
2. **核心缺失功能**：导航（AIS/航线/航迹）、坐标转换、海图解析、缓存、警报等核心功能未在JNI层暴露
3. **建议优先补充P0级API**，这些是海图应用的必备功能
4. **注意前置依赖**：部分API（如RouteManager、IParser）在C++层为桩实现，需先完善C++层再封装JNI
5. **ogc_recovery模块**在C++层完全未实现，需先完成C++层开发后再考虑JNI封装
