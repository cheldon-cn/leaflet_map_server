...
ParseResult S100Parser::ParseChart(const std::string& filePath, const ParseConfig& config) {
    ...
    LOG_INFO("Parsing S100 file: %s", filePath.c_str());
    ...
    LOG_INFO("S100 parsing completed. %d features parsed in %.2f ms", 
...
bool S100Parser::OpenDataset(const std::string& filePath, void** dataset) {
    ...
    if (!ds) {
        LOG_ERROR("Failed to open S100 file: %s", filePath.c_str());
        return false;
    }
