...
public:
    ...
    Impl() : minX(0), minY(0), maxX(1000), maxY(1000), srid(4326),
             pointCountDist(4, 8), initialized(false) {
        rng.seed(44);
    }
};

PolygonGenerator::PolygonGenerator() : m_impl(new Impl()) {
}

PolygonGenerator::~PolygonGenerator() {
}

MokRenderResult PolygonGenerator::Initialize(double minX, double minY,
...
void* PolygonGenerator::GenerateFeature() {
    ...
    double radius = 10.0 + (m_impl->rng() % 50);
    ...
    return polygonPtr.release();
}

std::string PolygonGenerator::GetLayerName() const {
    return "polygon_layer";
}

std::string PolygonGenerator::GetGeometryType() const {
    return "Polygon";
}
