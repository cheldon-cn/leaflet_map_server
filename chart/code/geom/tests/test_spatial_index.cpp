...
TEST_F(RTreeTest, Query_Envelope_ReturnsCorrectItems) {
    ...
    tree.Insert(Envelope(0, 10, 0, 10), 1);
    tree.Insert(Envelope(5, 15, 5, 15), 2);
    tree.Insert(Envelope(20, 30, 20, 30), 3);
    
    std::vector<int> results = tree.Query(Envelope(0, 15, 0, 15));
...
TEST_F(RTreeTest, Query_Point_ReturnsCorrectItems) {
    ...
    tree.Insert(Envelope(0, 10, 0, 10), 1);
    tree.Insert(Envelope(20, 30, 20, 30), 2);
...
TEST_F(RTreeTest, QueryNearest_ReturnsCorrectItems) {
    ...
    tree.Insert(Envelope(0, 1, 0, 1), 1);
    tree.Insert(Envelope(10, 11, 10, 11), 2);
    tree.Insert(Envelope(20, 21, 20, 21), 3);
...
TEST_F(RTreeTest, Remove_NonExistingItem_ReturnsFalse) {
    ...
    bool result = tree.Remove(Envelope(20, 30, 20, 30), 2);
    EXPECT_FALSE(result);
}
...
TEST_F(QuadtreeTest, Query_Envelope_ReturnsCorrectItems) {
    ...
    tree.Insert(Envelope(10, 20, 10, 20), 1);
    tree.Insert(Envelope(30, 40, 30, 40), 2);
    tree.Insert(Envelope(50, 60, 50, 60), 3);
    
    std::vector<int> results = tree.Query(Envelope(15, 35, 15, 35));
    EXPECT_EQ(results.size(), 2);
}
...
TEST_F(QuadtreeTest, QueryNearest_ReturnsCorrectItems) {
    ...
    tree.Insert(Envelope(10, 11, 10, 11), 1);
    tree.Insert(Envelope(30, 31, 30, 31), 2);
    tree.Insert(Envelope(50, 51, 50, 51), 3);
...
TEST_F(QuadtreeTest, Clear_RemovesAllItems) {
    ...
    for (int i = 0; i < 10; ++i) {
        tree.Insert(Envelope(i * 5, i * 5 + 5, i * 5, i * 5 + 5), i);
    }
...
TEST_F(GridIndexTest, Insert_MultipleItems_ReturnsSuccess) {
    ...
    for (int i = 0; i < 50; ++i) {
        Envelope env(i * 2, i * 2 + 1, i * 2, i * 2 + 1);
...
TEST_F(GridIndexTest, Query_Envelope_ReturnsCorrectItems) {
    ...
    index.Insert(Envelope(10, 20, 10, 20), 1);
    index.Insert(Envelope(30, 40, 30, 40), 2);
    index.Insert(Envelope(50, 60, 50, 60), 3);
    
    std::vector<int> results = index.Query(Envelope(15, 55, 15, 55));
...
TEST_F(GridIndexTest, Clear_RemovesAllItems) {
    ...
    for (int i = 0; i < 20; ++i) {
        index.Insert(Envelope(i * 5, i * 5 + 5, i * 5, i * 5 + 5), i);
    }
