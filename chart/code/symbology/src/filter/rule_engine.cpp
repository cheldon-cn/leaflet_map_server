...
draw::DrawResult RuleEngine::Render(const CNFeature* feature, draw::DrawContext& context, double scale) const {
    if (!feature) {
        return draw::DrawResult::kInvalidParameter;
    }
    ...
    if (symbolizers.empty()) {
        return draw::DrawResult::kSuccess;
    }
    
    draw::DrawResult result = draw::DrawResult::kSuccess;
    draw::DrawContextPtr contextPtr(&context, [](draw::DrawContext*) {});
    for (const auto& sym : symbolizers) {
        if (sym) {
            draw::DrawResult r = sym->Symbolize(contextPtr, geom.get());
            if (r != draw::DrawResult::kSuccess) {
                result = r;
            }
        }
    }
...
draw::DrawResult RuleEngine::Render(const Geometry* geometry, draw::DrawContext& context, double scale) const {
    if (!geometry) {
        return draw::DrawResult::kInvalidParameter;
    }
    ...
    for (const auto& sym : symbolizers) {
        if (sym) {
            draw::DrawResult r = sym->Symbolize(contextPtr, geometry);
            if (r != draw::DrawResult::kSuccess) {
                result = r;
            }
        }
