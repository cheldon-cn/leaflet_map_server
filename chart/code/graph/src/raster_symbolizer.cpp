...
DrawResult RasterSymbolizer::Symbolize(DrawContextPtr context, const Geometry* geometry) {
    return Symbolize(context, geometry, m_defaultStyle);
}

DrawResult RasterSymbolizer::Symbolize(DrawContextPtr context, const Geometry* geometry, const DrawStyle& style) {
    if (!context || !geometry) {
...
uint32_t RasterSymbolizer::InterpolateColor(double value, double minVal, double maxVal) const {
    if (m_colorMap.empty()) {
        ...
        return Color::FromRGBA(gray, gray, gray, 255).GetRGBA();
    }
    ...
    for (size_t i = 0; i < m_colorMap.size() - 1; ++i) {
        if (value >= m_colorMap[i].value && value <= m_colorMap[i + 1].value) {
            ...
            Color c1(m_colorMap[i].color);
            Color c2(m_colorMap[i + 1].color);
            ...
            return Color::FromRGBA(r, g, b, a).GetRGBA();
        }
    }
