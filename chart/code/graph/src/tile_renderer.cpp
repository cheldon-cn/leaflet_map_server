...
TileRenderer::TileRenderer(TilePyramidPtr pyramid)
    : m_pyramid(pyramid)
    , m_tileWidth(256)
    , m_tileHeight(256)
    , m_outputFormat("png")
...
void TileRenderer::SetTilePyramid(TilePyramidPtr pyramid) {
    m_pyramid = pyramid;
}

TilePyramidPtr TileRenderer::GetTilePyramid() const {
    return m_pyramid;
}

void TileRenderer::SetRuleEngine(RuleEnginePtr engine) {
    m_ruleEngine = engine;
}

RuleEnginePtr TileRenderer::GetRuleEngine() const {
    return m_ruleEngine;
}

void TileRenderer::SetTileCache(TileCachePtr cache) {
    m_cache = cache;
}

TileCachePtr TileRenderer::GetTileCache() const {
    return m_cache;
}
...
TileRenderResult TileRenderer::RenderTile(const TileKey& key) {
    return RenderTile(key, m_tileWidth, m_tileHeight);
}

TileRenderResult TileRenderer::RenderTile(const TileKey& key, int width, int height) {
...
TileRenderResult TileRenderer::DoRenderTile(const TileKey& key, int width, int height) {
    TileRenderResult result;
    result.key = key;
...
TileRenderResult TileRenderer::RenderTileFromFeatures(const TileKey& key, const std::vector<const CNFeature*>& features) {
    TileRenderResult result;
    result.key = key;
...
std::vector<TileRenderResult> TileRenderer::RenderTiles(const std::vector<TileKey>& keys) {
...
bool TileRenderer::RenderToImage(const TileKey& key, DrawContext& context) {
    if (!m_pyramid) {
        return false;
    }
...
std::vector<TileKey> TileRenderer::GetTilesForExtent(const Envelope& extent, double resolution) const {
    if (!m_pyramid) {
...
TileRendererPtr TileRenderer::Create(TilePyramidPtr pyramid) {
