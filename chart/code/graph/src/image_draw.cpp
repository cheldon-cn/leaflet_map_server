...
Color ImageData::GetPixel(int x, int y) const
{
    ...
    if (m_channels >= 4) {
        ...
        return Color(m_data[index], m_data[index + 1], m_data[index + 2], 255);
    } else if (m_channels == 1) {
        return Color(m_data[index], m_data[index], m_data[index], 255);
    }
...
void ImageData::Fill(const Color& color)
{
    if (!IsValid()) {
        return;
    }
...
void ImageDraw::SetPixel(int x, int y, const Color& color)
{
...
Color ImageDraw::GetPixel(int x, int y) const
{
    if (!IsValid()) {
        return Color();
    }
...
void ImageDraw::DrawLine(int x1, int y1, int x2, int y2, const Color& color, int thickness)
{
    DrawLineBresenham(x1, y1, x2, y2, color, thickness);
}

void ImageDraw::DrawLineAA(int x1, int y1, int x2, int y2, const Color& color, int thickness)
{
    DrawLineXiaolinWu(x1, y1, x2, y2, color, thickness);
}

void ImageDraw::DrawRect(int x, int y, int width, int height, const Color& color, int thickness)
{
...
void ImageDraw::FillRect(int x, int y, int width, int height, const Color& color)
{
    if (!IsValid()) {
        return;
    }
...
void ImageDraw::DrawCircle(int cx, int cy, int radius, const Color& color, int thickness)
{
    DrawCircleMidpoint(cx, cy, radius, color, thickness);
}

void ImageDraw::FillCircle(int cx, int cy, int radius, const Color& color)
{
    FillCircleScanline(cx, cy, radius, color);
}

void ImageDraw::DrawEllipse(int cx, int cy, int rx, int ry, const Color& color, int thickness)
{
    DrawEllipseMidpoint(cx, cy, rx, ry, color, thickness);
}

void ImageDraw::FillEllipse(int cx, int cy, int rx, int ry, const Color& color)
{
    FillEllipseScanline(cx, cy, rx, ry, color);
}

void ImageDraw::DrawPolygon(const std::vector<std::pair<int, int>>& points, const Color& color, int thickness)
{
    if (points.size() < 2) {
        return;
    }
...
void ImageDraw::FillPolygon(const std::vector<std::pair<int, int>>& points, const Color& color)
{
    FillPolygonScanline(points, color);
}

void ImageDraw::DrawText(int x, int y, const std::string& text, const Color& color, const std::string& fontFamily, int fontSize)
{
    (void)x;
    (void)y;
    (void)text;
...
void ImageDraw::DrawImage(int x, int y, const ImageData* image, int srcX, int srcY, int srcWidth, int srcHeight, double opacity)
{
    ...
    for (int dy = 0; dy < srcHeight; ++dy) {
        for (int dx = 0; dx < srcWidth; ++dx) {
            ...
            Color srcColor = image->GetPixel(srcX + dx, srcY + dy);
            Color dstColor = m_imageData->GetPixel(dstX, dstY);
            ...
            m_imageData->SetPixel(dstX, dstY, Color(r, g, b, a));
        }
    }
}

void ImageDraw::Blend(int x, int y, const Color& color, double alpha)
{
    if (!IsValid() || !IsPointInClipRect(x, y)) {
        return;
    }
    
    Color dstColor = m_imageData->GetPixel(x, y);
    ...
    m_imageData->SetPixel(x, y, Color(r, g, b, a));
}

void ImageDraw::Clear(const Color& color)
{
    if (!IsValid()) {
        return;
    }
...
void ImageDraw::DrawLineBresenham(int x1, int y1, int x2, int y2, const Color& color, int thickness)
{
...
void ImageDraw::DrawLineXiaolinWu(int x1, int y1, int x2, int y2, const Color& color, int thickness)
{
    (void)thickness;
...
void ImageDraw::DrawCircleMidpoint(int cx, int cy, int radius, const Color& color, int thickness)
{
...
void ImageDraw::FillCircleScanline(int cx, int cy, int radius, const Color& color)
{
...
void ImageDraw::DrawEllipseMidpoint(int cx, int cy, int rx, int ry, const Color& color, int thickness)
{
    for (int t = 0; t < thickness; ++t) {
        int currentRx = rx - t;
        int currentRy = ry - t;
...
void ImageDraw::FillEllipseScanline(int cx, int cy, int rx, int ry, const Color& color)
{
    for (int y = -ry; y <= ry; ++y) {
...
void ImageDraw::FillPolygonScanline(const std::vector<std::pair<int, int>>& points, const Color& color)
{
    if (points.size() < 3) {
        return;
    }
