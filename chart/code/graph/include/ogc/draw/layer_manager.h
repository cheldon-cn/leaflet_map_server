...
public:
    ...
    virtual DrawResult Render(CNLayer* layer, DrawContext& context) = 0;
    virtual DrawResult RenderSelection(CNLayer* layer, DrawContext& context, 
                                        const std::vector<int64_t>& featureIds) = 0;
    virtual DrawResult RenderLabels(CNLayer* layer, DrawContext& context) = 0;
};
