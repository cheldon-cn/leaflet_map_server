...
class OGC_CACHE_API TilePyramid {
public:
    TilePyramid();
    ...
    void SetExtent(const ogc::Envelope& extent);
    ogc::Envelope GetExtent() const;
    
    ogc::Envelope GetTileExtent(const TileKey& key) const;
    ogc::Envelope GetTileExtent(int x, int y, int z) const;
...
namespace std {
template<>
struct hash<ogc::cache::TileKey> {
    size_t operator()(const ogc::cache::TileKey& key) const {
