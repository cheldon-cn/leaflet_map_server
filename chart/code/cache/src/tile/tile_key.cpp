...
void TilePyramid::SetExtent(const ogc::Envelope& extent) {
    m_extent = extent;
}

ogc::Envelope TilePyramid::GetExtent() const {
    return m_extent;
}
...
ogc::Envelope TilePyramid::GetTileExtent(const TileKey& key) const {
    return GetTileExtent(key.x, key.y, key.z);
}

ogc::Envelope TilePyramid::GetTileExtent(int x, int y, int z) const {
...
std::vector<TileKey> TilePyramid::GetTilesForExtent(const ogc::Envelope& extent, int z) const {
...
TilePyramidPtr TilePyramid::CreateWebMercator() {
    ...
    pyramid->SetExtent(ogc::Envelope(-20037508.34, -20037508.34, 20037508.34, 20037508.34));
    return pyramid;
}
...
TilePyramidPtr TilePyramid::CreateGeodetic() {
    ...
    pyramid->SetExtent(ogc::Envelope(-180.0, -90.0, 180.0, 90.0));
    return pyramid;
}
