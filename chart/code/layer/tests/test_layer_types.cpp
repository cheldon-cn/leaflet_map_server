...
class CNRasterDatasetTest : public ::testing::Test {
protected:
};

TEST_F(CNRasterDatasetTest, OpenNull) {
    auto ds = CNRasterDataset::Open("nonexistent.tif", false);
    EXPECT_EQ(ds, nullptr);
}

TEST_F(CNRasterDatasetTest, CreateNull) {
    auto ds = CNRasterDataset::Create("test.tif", 100, 100, 1, CNDataType::kByte);
    EXPECT_EQ(ds, nullptr);
}
