...
CNShapefileLayer::CNShapefileLayer()
    : impl_(new Impl()) {
}

CNShapefileLayer::~CNShapefileLayer() = default;

std::unique_ptr<CNVectorLayer> CNShapefileLayer::Open(
...
std::unique_ptr<CNVectorLayer> CNShapefileLayer::Create(
    const std::string& path, GeomType geom_type) {
    (void)path;
    (void)geom_type;
    return nullptr;
}
...
std::string CNShapefileLayer::GetFormatName() const {
    return "ESRI Shapefile";
}

bool CNShapefileLayer::IsReadOnly() const {
    return impl_->is_read_only_;
}

CNFeatureDefn* CNShapefileLayer::GetFeatureDefn() {
    return impl_->feature_defn_.get();
}

const CNFeatureDefn* CNShapefileLayer::GetFeatureDefn() const {
    return impl_->feature_defn_.get();
}

GeomType CNShapefileLayer::GetGeomType() const {
...
void* CNShapefileLayer::GetSpatialRef() {
    return impl_->spatial_ref_;
}

const void* CNShapefileLayer::GetSpatialRef() const {
    return impl_->spatial_ref_;
}

CNStatus CNShapefileLayer::GetExtent(Envelope& extent, bool force) const {
    (void)force;
    extent = impl_->extent_;
    return CNStatus::kSuccess;
}

int64_t CNShapefileLayer::GetFeatureCount(bool force) const {
    (void)force;
    return static_cast<int64_t>(impl_->features_.size());
}

void CNShapefileLayer::ResetReading() {
    impl_->read_cursor_ = 0;
}

std::unique_ptr<CNFeature> CNShapefileLayer::GetNextFeature() {
...
CNFeature* CNShapefileLayer::GetNextFeatureRef() {
...
std::unique_ptr<CNFeature> CNShapefileLayer::GetFeature(int64_t fid) {
...
CNStatus CNShapefileLayer::SetFeature(const CNFeature* feature) {
    (void)feature;
    return CNStatus::kNotSupported;
}

CNStatus CNShapefileLayer::CreateFeature(CNFeature* feature) {
    (void)feature;
    return CNStatus::kNotSupported;
}

CNStatus CNShapefileLayer::DeleteFeature(int64_t fid) {
    (void)fid;
    return CNStatus::kNotSupported;
}

int64_t CNShapefileLayer::CreateFeatureBatch(const std::vector<CNFeature*>& features) {
    (void)features;
    return 0;
}

CNStatus CNShapefileLayer::CreateField(const CNFieldDefn* field_defn, bool approx_ok) {
    (void)field_defn;
    (void)approx_ok;
    return CNStatus::kNotSupported;
}

CNStatus CNShapefileLayer::DeleteField(int field_index) {
    (void)field_index;
    return CNStatus::kNotSupported;
}

void CNShapefileLayer::SetSpatialFilter(const CNGeometry* geometry) {
    if (geometry) {
...
const CNGeometry* CNShapefileLayer::GetSpatialFilter() const {
    return impl_->spatial_filter_.get();
}

CNStatus CNShapefileLayer::SetAttributeFilter(const std::string& query) {
    impl_->attribute_filter_ = query;
    return CNStatus::kSuccess;
}

bool CNShapefileLayer::TestCapability(CNLayerCapability capability) const {
    switch (capability) {
...
std::unique_ptr<CNLayer> CNShapefileLayer::Clone() const {
    return nullptr;
}
