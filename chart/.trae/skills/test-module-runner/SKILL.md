...
description: "Compiles and runs module tests with 5-second timeout control (default rule: skip tests exceeding 5s). Invoke when user asks to run tests, execute test cases, or test specific modules like 'code/graph/tests'."
---

# Test Module Runner

Automates the compilation, execution, and reporting of module tests with **5-second timeout control** (default rule: skip tests exceeding 5 seconds).
...
5. **默认超时规则**: 单个用例执行时间超过5秒自动跳过（无需用户显式指定）
...
| test_yyy.exe | - | ⏭️ SKIPPED | [测试功能描述] | 执行超过5秒，见问题分析 |
```

**注意**：
- 必须包含"测试描述"列，简要描述测试了什么功能
- 对于跳过的测试，需要在"备注"列标注原因
- 如果是跳过执行的测试，需要提供详细的问题分析

### Problem Analysis (for Skipped Tests)
...
实施以上优化后，预计测试执行时间可从超过5秒降低到[X]秒以内。
```
...
Use `Wait-Job -Timeout 5` to implement 5-second timeout:
...
| Timeout not working | Not using Wait-Job | Use `Wait-Job -Timeout 5` |
...
| 测试文件 | 测试数 | 状态 | 测试描述 | 备注 |
|----------|--------|------|----------|------|
| test_graph_boundary.exe | - | ⏭️ SKIPPED | 边界值测试 | 执行超过5秒，见问题分析 |
